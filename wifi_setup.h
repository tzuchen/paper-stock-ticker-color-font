...
static const int EEPROM_OFFSET = 16; // prefs shim 使用 0..3，避免互相覆蓋
...
inline void portal() {
  ...
  server.on("/", HTTP_GET, [&server]() {
    ...
    for (int i = 0; i < n; ++i) {
      ...
      options += "<option value=\"" + htmlEscape(name) + "\">" + htmlEscape(name) +
                 " (" + String(WiFi.RSSI(i)) + " dBm)" + "</option>";
    }
    WiFi.scanDelete();
    String page = F("<!doctype html><html><meta name='viewport' content='width=device-width,initial-scale=1'>"
                    "<title>SPY Ticker Wi-Fi</title><style>body{font:18px sans-serif;max-width:480px;margin:32px auto;padding:0 18px}"
                    "input,select,button{font-size:18px;width:100%;padding:10px;margin:8px 0}button{background:#111;color:white}</style>"
                    "<h2>SPY Ticker Wi-Fi 設定</h2><p>選擇家中 Wi-Fi 並輸入密碼，儲存後裝置會自動重啟。</p>"
                    "<form method='POST' action='/save'><label>Wi-Fi</label><select name='ssid'>");
    page += options;
    page += F("</select><label>密碼</label><input name='password' type='password' autocomplete='off'>"
               "<button type='submit'>儲存並重啟</button></form><p>若看不到網路，請靠近路由器後重新整理。</p></html>");
...
#else
  const char* ssid = fallbackSsid;
  const char* password = fallbackPassword;
#endif
